---
title: "Assignment 1 - Data Cleaning - Autist Data"
...
```{r}
#renaming the columns "SUBJ", "Child.ID"" and "Visit" in different datasets
...
#making words "visit1/2..." apear as a digit "1", "2" etc.

LU$VISIT = str_extract(LU$VISIT, "\\d")
LU$VISIT<- as.numeric(LU$VISIT,"\\d")
...
```{r}
#getting rid of punctuation marks in ID columns 
...
#creating datasets/subsests with only the variables we need 

subDemo <- select(demo,ID,VISIT,Diagnosis,Gender,Age,MullenRaw,ExpressiveLangRaw,ADOS,Socialization, Ethnicity)
...
#merging the subsets created previously

subDemo$VISIT <- as.numeric(subDemo$VISIT)
class(subDemo$VISIT)

Clean_data <- merge(subDemo,subToken, subLU, by.x = c("ID", "VISIT"), by.y = c("ID", "VISIT"), all=TRUE)



#2nd version
#Clean_data_1<- as.data.frame(full_join(subLU, subToken))
#subDemo$VISIT <- as.numeric(subDemo$VISIT)
#class(subDemo$VISIT)
#Clean_data_1$VISIT <-  as.numeric(Clean_data_1$VISIT)
#Clean_data <- full_join(Clean_data_1, subDemo)
...
#creating a dataset with only data from 1st visit to selected variables
 firstVisit <- filter(Clean_data, VISIT == 1)
firstVisit <- select(firstVisit,ID,ADOS,MullenRaw,ExpressiveLangRaw,Socialization)

#renaming the newly created columns of 1st visit dataset adding digit "1" at the end of the original name
...
#merging the previously cleaned dataset with the newly created first visit dataset getting 18 variables in total
finalData1 <- merge(Clean_data,firstVisit, by = "ID")



#2nd way
#VISIT_1_1 <- data.frame(Clean_data$ID, Clean_data$ADOS, Clean_data$MullenRaw, #Clean_data$ExpressiveLangRaw, Clean_data$Socialization, Clean_data$VISIT == 1)
```
...
#changing gender variable from digits to leters "F" and "M"
finalData1$Gender[finalData1$Gender=="1"] <- "F"
finalData1$Gender[finalData1$Gender=="2"] <- "M"

#making Diagnosis variable as.character so it allows the next step of changing leters "A" and "B" to diagnosis abbreviations
...
#anonymizing our participants by changing ID column values to factor level and then changing it to numeric values, what automatically gives values 1,2,3.. etc.
finalData1$ID <- as.factor(finalData1$ID)
finalData1$ID <- as.numeric(finalData1$ID)

#saving file in my directory folder as .csv 
write.csv(finalData1, "autistData.csv")
